...
;(function(window) {
	...
	var bodyEl = document.body, 
		...
		dispCtrl = sliderEl.querySelector('.button--comment'),/* add */
	...
	function initEvents() {
		...
		// add disp comment
		dispCtrl.addEventListener('click', function() {
			openComment(items[current]);
		});
	...
	function openComment(item) {
        ...
        function modalResize(){
            ...
            // モーダルコンテンツの表示位置を取得
            var x = (w - $(id).outerWidth(true)) / 2;
            var y = (h - $(id).outerHeight(true)) / 2;
